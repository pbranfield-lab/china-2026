...
**Last updated: 2026-08-09 05:30.** Read this, then `CLAUDE.md` for the
architecture detail.
...
## The cloisonné re-skin — implemented, not yet visually verified

All eleven tasks of `docs/superpowers/plans/2026-08-09-cloisonne-reskin.md` are
implemented and committed on the branch **`cloisonne-reskin`**, at
`SITE_VERSION 2.0.0`.

⚠️ **It has not been opened in a browser.** `node tools/check.mjs` passes all 13
checks and the contrast maths was computed rather than eyeballed, but every
*visual* claim below is unverified: nothing has been looked at, clicked, or
loaded at a mobile width. The plan's browser matrix (Task 11, Step 2 — fifteen
page loads plus a sixteen-item regression checklist) is the outstanding work.
**Do that before merging to `master`.**

### What changed

- **Palette and type.** The lacquer/vermillion/gold tokens are gone, replaced by
  a cloisonné enamel palette — deep teal grounds, a gold wire hairline, coral and
  jade fields, mint and cream for type on dark. Three new Latin families:
  Fraunces 900 (`--font-display`), Outfit (`--font-ui`), Nunito (`--font-body`).
  Noto Serif SC stays and is now named explicitly by every hanzi element, because
  the Latin faces carry no CJK.
- **One enamel plate recipe** replaces the corner-bracket primitive and is shared
  by every raised surface — nav cards, family cards, the location popout, the
  modal, the bonus feature and the fact tiles.
- **The facts are now teasers.** Tiles carry only the index, `stat`, `label` and a
  hint; clicking one opens `#factOverlay`, a `role="dialog"` popout with the fact
  in quotes over a coral plate and Maisie's medallion beneath it. Arrows, dots
  and ←/→ step through all ten and wrap; Escape, ✕ or a backdrop click closes and
  returns focus to the tile. Tab is trapped inside.
- **`assets/maisie-avatar.jpg`** — new, 240×240, 13 KB. Cropped from
  `Photographs/the-great-wall/maisie-portrait.jpg` at `(0, 275, 410, 685)`, which
  is tighter than the box in the plan: it keeps bystanders out of frame and suits
  a 46px circular medallion. One photo serves all three trips — the Xi'an folder
  contains no photograph of Maisie.
- **`SITE_VERSION` → `2.0.0`.** The stamp bottom-left should read `v2.0.0`.

### Two deliberate deviations from the plan

1. **Contrast.** The plan's Step 3 table measured `--mint`/`--cream`/`--gold`
   against the *middle* stop of each gradient. Measured at the point where the
   text actually sits — a 158deg gradient puts its lightest stop directly under
   the first line — the coral and jade fields failed WCAG AA, some badly
   (mint on jade: 2.81, needed 4.5). Rather than flatten the palette, prose-bearing
   surfaces now use darker variants (`--coral-field/-2/-3`, `--coral-plate`,
   `--jade-field/-2`); pins, close buttons, play badges and the seal keep the
   bright coral because they carry a glyph, not prose. Every pair now passes —
   the full table of final ratios is in the commit message for `cc55da0`.
2. **Count-up grouping.** The plan's `countUp()` always grouped thousands, so the
   two year stats (`1974`, `1368`) would have counted up as `1,974` / `1,368` and
   then snapped to the un-grouped string on the last frame. It now groups only if
   the written `stat` does. All 24 counting stats were verified to format back to
   their exact source string.

### Don't regress these — additions from this work

Carried forward from the list further down, plus three new ones:

- **Direct children of an enamel plate need `position:relative`.** The lattice
  overlay is an absolutely-positioned `::before`, so ordinary in-flow children —
  including photographs — paint underneath it. One grouped rule handles every
  plate class; extend it when adding a new one.
- **The map's cream mat is `box-shadow` spread, never padding.** `#pinLayer`
  positions pins as percentages of `.map-inner`; padding there shifts every pin
  off its landmark.
- **The count-up needs its own `matchMedia` guard.** It changes `textContent`,
  which the stylesheet's `prefers-reduced-motion` blanket cannot suppress.
- **`--coral` and `--jade` are not text grounds** — see the contrast note above.

### Picking it up

```bash
git checkout cloisonne-reskin
node tools/check.mjs          # expect: all 13 checks pass
python3 -m http.server 8000
```

Then work the Task 11 Step 2 matrix in the plan: five pages × three trips, plus
the sixteen-item regression checklist (trip routing, video audio on close, CC
credits, pin placement, the compass hidden on the Great Wall, the mobile nav).
Fix anything that fails, then merge to `master` and push — pushing to `master`
publishes.

⚠️ `git remote -v` must say **`china-2026`**. The old `forbidden-city-maisie`
remote is a redirect stub and pushing there publishes nothing here.

---

**Everything below this line describes the site as it was before the re-skin.**
It is still accurate about how the site works — data model, routing, photo
curation, content rules — and only the visual layer changed. Live at
...
Shipped as `5e244b8`, pushed to `origin/master`, so it is live. **It was not
checked in a browser** — `node --check` passed on both JS files and a data script
confirmed `version 1.4.1 | credited 3 of 94` with all three files present on
disk, but nobody has actually looked at the rendered credit lines. Paul chose to
commit as-is. Worth one look at
`gallery.html?trip=xian` (the credited photos are `bell-tower-day`,
`muslim-quarter-great-mosque-sign`, `muslim-quarter-xiyangshi-arch`).
...
| | |
|---|---|
| `5e244b8` | Visible CC attribution for the three sourced Xi'an photos (7 files) |
| `7c3f227` | Brought the handoff and architecture docs up to date |
...
It caught the `const`-in-`vm` gotcha and confirms all 94 photo files resolve on
disk. **It lives in the session scratchpad, not the repo — which means it is
gone in a fresh session and has to be rewritten.** That has now happened twice.
Moving it into the repo would fix that permanently, but it changes the "no build
tooling" shape of the project, so **ask Paul before doing so**. The full list of
checks is below, which is enough to rewrite it from scratch.

⚠️ When rewriting it, remember the gotcha: `data.js` declares everything with
`const`, so `vm.runInNewContext` won't expose the bindings. Load it with
`new Function(src + ';return {TRIPS,LOCATIONS,PHOTOS,FACTS,FAMILY,SITE_VERSION};')()`.
...
git log --oneline -3       # newest should be 5e244b8 if nothing has happened since
node --check assets/data.js && node --check assets/site.js
ls docs/HANDOFF-*.md       # the timestamp in the name tells you how stale this is
```
...
**Every one of these needs Paul, not code.** That's why the last session went
looking for the CC item instead — it was the only one that didn't.

1. **Get the identity table in `docs/great-wall-moments.md` confirmed.** Names
   in those captions are inferred from outfits. This is live and public. Chase
   this one first.
2. **Source URLs for the three sourced Xi'an photos**, so a `source` link can be
   added to each `credit` object and full TASL attribution completed.
3. Chase the Great Wall family moments / William quotes — that trip currently
   has none by design.
4. One manual click of the facts jump button (see verification section).
5. One look at the rendered CC credit lines on `gallery.html?trip=xian` — the
   code shipped unverified in a browser.
6. ~~Consider visible CC attribution for the three sourced Xi'an photos.~~ Done
   — but source URLs are still missing, see item 2 and "Loose ends".

---

## Closing out a session

Do this at the end, so the next session starts clean:

1. Update this file — state, shipped commits, loose ends, what needs Paul.
2. **Rename it to the current timestamp** and fix the two pointers:
   ```
...
3. Update `CLAUDE.md` if the *architecture* changed — that file describes how the
   site is built and is meant to stay true indefinitely; this file describes
   where the work got to and is expected to go stale.
