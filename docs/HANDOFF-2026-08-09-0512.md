...
**Last updated: 2026-08-09 05:05.** Read this, then `CLAUDE.md` for the
architecture detail.
...
## ⚠️ IN PROGRESS — the cloisonné re-skin

**Start here, not at the section below.** A full visual re-skin is designed,
planned and part-implemented. `SITE_VERSION` is still 1.4.1 and the live site is
still the old lacquer look — nothing visual has shipped yet.

- **Design (approved):** `docs/superpowers/specs/2026-08-09-cloisonne-reskin-design.md`
- **Plan (11 tasks, complete code in every step):** `docs/superpowers/plans/2026-08-09-cloisonne-reskin.md`
- **Done:** Task 1 only — `tools/check.mjs`, a structural check harness.
  Run `node tools/check.mjs`. It currently reports **exactly two failures**
  (missing CSS tokens, missing facts popout ids). That is the correct baseline:
  those two go green in Task 2 and Task 8 respectively. Any *other* failure is a
  regression.
- **Next: Task 2** — the palette and typography swap. Read the plan and follow it;
  don't re-derive the design. Execute with `superpowers:executing-plans` (or
  subagent-driven).

### Two things changed underneath the plan since it was written

Both landed after Task 1 and **neither is in the plan's task list**, so read this
before Task 2 or the file paths won't match what you expect.

1. **`assets/data.js` was split** (commit `f49ae57`). It was 127 KB in one file;
   it is now a 4.4 KB assembler plus `assets/data/forbidden-city.js`,
   `assets/data/xian.js` and `assets/data/great-wall.js`. Each trip file declares
   one `TRIP_*` object (`trip`, `facts`, `locations`, `photos`); `data.js` holds
   `SITE_VERSION`, `ofTrip()`, `FAMILY`, `EMPTY_STATE_LINES` and stitches
   `TRIP_MODULES` into the flat `TRIPS` / `LOCATIONS` / `PHOTOS` / `FACTS`
   globals. All five pages now load four data scripts, trip files first.
   The split was proved equivalent — all seven globals byte-identical — so
   `PHOTOS` keeps the positional order the map popout's `PHOTOS.indexOf()` needs.
   **Impact on the plan:** Task 11 Step 1 says to bump `SITE_VERSION` at
   `assets/data.js:4`. It is still in `assets/data.js`, just at a different line.
2. **`CLAUDE.md` was slimmed** (commits `b576ca2`, `4b1c7ac`) from 16.5 KB to
   10.4 KB, and the content-authoring guidance moved to `docs/authoring.md`.
   **Impact on the plan:** Task 11 Step 3 quotes three passages of `CLAUDE.md` to
   replace. The "no lint, test, or build command" sentence it targets is already
   gone — that step's (a) is done. Its (b) and (c) still apply but the surrounding
   wording differs; match on meaning, not on the exact quoted text.

The parent `C:\Claude\CLAUDE.md` was also slimmed (13,977 → 3,115 bytes, detail
moved to `C:\Claude\notes\`), which is why sessions now start with far less
context. Backup at `C:\Claude\CLAUDE.md.bak-20260809-0450`.

Everything below this line describes the state **before** the re-skin started. It
is still accurate about how the site behaves, but note the data-layer paths above
supersede any mention of a single `assets/data.js` containing all the content.

---

**State before the re-skin: everything done, verified, committed, pushed and
live.** Live at `https://pbranfield-lab.github.io/china-2026/`.
⚠️ `master` is **ahead of `origin/master` and nothing has been pushed** — the
spec, the plan, the check harness, the data split and the CLAUDE.md work are all
local only. Check with `git rev-list --count origin/master..master`. The plan
pushes at Task 11; don't push a half-finished re-skin before then.
