...
- Attribution for sourced photos: `creditBadge(photo)` appends a plain-text credit inside the thumbnail (plain text because a thumb is a `<button>`, and a nested `<a>` is invalid there), and the modal fills `#modalCredit` with the full line plus a link to the licence deed. `#modalCredit` is `hidden` in the markup and only unhidden when the photo has a `credit`, so family photos get no stray divider. CC BY / BY-SA want the attribution visible wherever the work is shown, which is why it's on both and not just in this file.
...
**Sourced (non-family) photos** must not mention any family member and must stay factual. Currently: `bell-tower-day.jpg` (Wang Zhongyin, CC BY-SA 4.0), `muslim-quarter-great-mosque-sign.jpg` (Qianeal, CC BY-SA 4.0), `muslim-quarter-xiyangshi-arch.jpg` (thierrytutin, CC BY 2.0). All three are Xi'an; the Great Wall trip is entirely family photos. Each carries a `credit` object in `PHOTOS` so the attribution is rendered on the page — **a new sourced photo needs one, listing it here is not sufficient.** The `credit` objects have no `source` URL yet; the originals' URLs were never recorded, so full TASL attribution is still incomplete.
